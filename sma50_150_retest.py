...
# ─────────────────────────────────────────────────────────────
# PIPELINE CONNECTOR — run_scan() entry point
# Called by pipeline_connector.py to get tiered results
# ─────────────────────────────────────────────────────────────

def _run_core_scan(tickers):
    """Runs scan loop on given ticker list, returns results sorted by Score."""
    global _hdr_done
    _hdr_done = False
    scan_results = []
        ...
        for batch in batches:
            ...
            for sym in batch:
                ...
                try:
                    ...
                    if r: scan_results.append(r); live_print(r)
                except Exception: pass
...
def run_scan() -> dict:
    """
    Entry point for pipeline_connector.py.
    Returns tiered dict: {"T1": [...], "T2": [...], "T3": [...]}
    Tier mapping uses Retest_Type (this scanner already has natural tiers):
      T1 = SMA150 + SMA50 retest  (Score >= 60 or both MAs tested)
      T2 = SMA150 retest only     (Score 40-59)
      T3 = SMA50 retest only      (Score < 40)
    """
    tickers = get_tickers()
    scan_results = _run_core_scan(tickers)
    tiers = {"T1": [], "T2": [], "T3": []}
    for r in scan_results:
        score  = r.get("Score", 0)
        ticker = r["Ticker"]
        rtype  = r.get("Retest_Type", "")
        if "SMA150 + SMA50" in rtype or score >= 60:
            tiers["T1"].append(ticker)
        elif "SMA150" in rtype or score >= 40:
            tiers["T2"].append(ticker)
        else:
            tiers["T3"].append(ticker)
    t1 = tiers["T1"]; t2 = tiers["T2"][:5]; t3 = tiers["T3"][:5]
    print(f"\n  run_scan() complete → T1={t1}  T2={t2}  T3={t3}")
    return tiers
