...
def run_scan() -> dict:
    ...
    Returns tiered dict: {"T1": [...], "T2": [...], "T3": [...]}
      T1 = Score >= 70  (strong setup — goes to Research Agent)
      T2 = Score 50-69  (medium conviction)
      T3 = Score 30-49  (watchlist only)
    """
    tickers = get_tickers()
    ...
    for r in scan_results:
        score  = r.get("Score", 0)
        ticker = r["Ticker"]
        if score >= 70:   tiers["T1"].append(ticker)
        elif score >= 50: tiers["T2"].append(ticker)
        elif score >= 30: tiers["T3"].append(ticker)
