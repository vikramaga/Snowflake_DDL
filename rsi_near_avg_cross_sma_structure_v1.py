# ============================================================
# NASDAQ — RSI Near/Crossing Its Average (From Below) Scanner (v1)
# ============================================================
#
# SIGNAL (all required, scanned over the last signal_lookback_days
# trading days, not just today):
#
#   1. RSI TRIGGER: RSI(14) was BELOW its own moving average
#      (RSI's rsi_avg_period-bar SMA) on the PREVIOUS day — confirms
#      it's approaching FROM BELOW, not just incidentally close —
#      AND today it is either very close to that average (within
#      near_tolerance points, even if still slightly below) OR has
#      already crossed above it. This captures the transition
#      moment itself, not a persistent already-above state.
#
#   2. AVERAGE RSI STRONG: RSI's own moving average is currently
#      above rsi_avg_min_level (50) — the broader momentum backdrop
#      is healthy, not just a bounce inside a weak/declining trend.
#
#   3. STRUCTURE: price is above SMA50 AND SMA150, AND SMA50 is
#      above SMA150 (bullish structure).
#
...
def check_pattern_at(df, rsi, rsi_avg, sma50, sma150, i, cfg):
    """
    ...
    ra_now, ra_prev = rsi_avg.iloc[i], rsi_avg.iloc[i-1]
    s50, s150 = sma50.iloc[i], sma150.iloc[i]
    if any(np.isnan(v) for v in [r_now, r_prev, ra_now, ra_prev, s50, s150]):
        return False, {}

    close_i = float(df["Close"].iloc[i])

    # ── Step 1: RSI near/crossing its own average, from below ──────
    was_below = r_prev < ra_prev
    gap_now = float(r_now - ra_now)
    ...
    structure_ok = (close_i > s50) and (close_i > s150) and (s50 > s150)

    passed = rsi_trigger_ok and rsi_avg_strong and structure_ok
...
# ── Technical signal: RSI near/crossing its average from below ────
def analyze_rsi_avg_cross(sym, df_daily):
    """
    ...
    hits = find_rsi_avg_cross_signals(df_daily, rsi, rsi_avg, sma50, sma150, CFG)
    if not hits:
        return None
    ...
    stop_loss   = float(df_daily["Low"].iloc[max(0,sig_idx-9):sig_idx+1].min())
    ...
    score = 0
    reasons = []
    score += max(0, min(30, 30 - abs(sig["gap_now"]) * (30/CFG["near_tolerance"])))
    reasons.append(f"RSIGap{sig['gap_now']:+.1f}")
    score += min(25, (sig["rsi_avg_now"] - CFG["rsi_avg_min_level"]))
    reasons.append(f"RSIAvg{sig['rsi_avg_now']:.0f}")
    price_above_sma50_pct = (sig["close"] - sig["sma50"]) / sig["sma50"] * 100 if sig["sma50"]>0 else 0
    score += max(0, min(15, price_above_sma50_pct * 2))
    reasons.append(f"AboveSMA50+{price_above_sma50_pct:.1f}%")
    ...
    return {
        ...
        "RSI"            : round(sig["rsi_now"], 1),
        "RSI_Avg"        : round(sig["rsi_avg_now"], 1),
        "RSI_Gap"        : round(sig["gap_now"], 2),
...
print("  A stock only matches if the RSI trigger, average-RSI strength,")
...
for sym in tqdm(list(daily_map.keys()), desc="Checking RSI reversal", unit="stk"):
    try:
        r = analyze_rsi_avg_cross(sym, daily_map[sym])
...
COLS = [
    ...
    "RSI","RSI_Avg","RSI_Gap","SMA50","SMA150",
...
if _IN_NOTEBOOK and results:
    DISP = ["Ticker","Price","Score","Entry_Price","Stop_Loss",
            "RSI","RSI_Avg","RSI_Gap","Signal_Date"]
...
<div style="margin:14px 0">
  <div style="background:linear-gradient(90deg,{gc}22,#0f172a);
    ...
    <span style="color:#f1f5f9;font-size:15px;font-weight:700">RSI Near/Crossing Its Average</span>
...
<div style="background:linear-gradient(135deg,#0f172a,#1e3a5f);
  ...
  <h2 style="margin:0;color:#60a5fa;font-size:20px;font-weight:700">
    📈 RSI Near/Crossing Its Average (From Below)
  </h2>
...
<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;
  ...
  RSI(14) was below its own {CFG['rsi_avg_period']}-period average yesterday,
  and today is very near (within {CFG['near_tolerance']:.0f} points) or has
  crossed above it &nbsp;·&nbsp;
  RSI's own average is above {CFG['rsi_avg_min_level']:.0f} &nbsp;·&nbsp;
  Price above SMA50 and SMA150, SMA50 above SMA150 &nbsp;·&nbsp;
  ...
  last {CFG['signal_lookback_days']} trading days, not just today)
    ...
    print()
    print(f"  ╔{'═'*inner}╗")
    tit = f"  RSI Near/Crossing Its Average   {datetime.today().strftime('%Y-%m-%d')}   {len(df_out)} matches"
  ...
  COLUMN KEY
  ──────────────────────────────────────────────────────
  Score           0-100 (RSI-gap tightness + RSI-avg strength +
                  distance above SMA50 + freshness)
  Entry_Price     the signal day's close
  Stop_Loss       a 10-day swing-low proxy
  RSI             RSI(14) on the signal day
  RSI_Avg         RSI's own {CFG['rsi_avg_period']}-period moving average on the signal day
  RSI_Gap         RSI - RSI_Avg (negative = still just below, near 0 =
                  right at the cross)
...
# Save
fpath = os.path.join(out_dir, f"rsi_near_avg_cross_sma_structure_{ts}.csv")
df_out.to_csv(fpath, index=False)
print(f"\n  💾 CSV → {fpath}")
tv = os.path.join(out_dir, f"tv_rsi_near_avg_cross_sma_structure_{ts}.txt")
with open(tv,"w") as f:
    f.write(f"###RSI Near/Crossing Its Average {datetime.today().strftime('%Y-%m-%d')}\n")
...
<p style="margin:0;color:#94a3b8;font-size:10px">
  ⚠️ Not financial advice &nbsp;·&nbsp; Auto-generated by GitHub Actions
        ...
        if rl:
            for r in rl[:50]:
                ...
                plain_lines.append(
                    ...
                    f"RSI:{float(rsi_v):.1f}  RSIAvg:{float(rsi_a):.1f}  Gap:{float(rsi_g):+.2f}  Signal:{sdate}"
                )
            plain_lines.append("")
        ...
        subj = (f"📊 RSI Near/Crossing Its Average — {cnt} signal{'s' if cnt!=1 else ''}"
...
# ── Charts for top 5 (price + SMA50/SMA150 + RSI-vs-avg panel) ──
if results:
    ...
    for row, r in zip(axes, top):
        ...
        ax.set_title(
            ...
            f"RSI {r['RSI']:.0f} vs Avg {r['RSI_Avg']:.0f}  |  {r['Signal_Date']}",
    ...
    plt.suptitle(
        f"RSI Near/Crossing Its Average  ·  {datetime.today().strftime('%Y-%m-%d')}",
        color="#60a5fa", fontsize=12, fontweight="bold", y=1.001)
    plt.tight_layout()
    cp = os.path.join(out_dir, f"rsi_near_avg_cross_sma_structure_chart_{ts}.png")
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  1) RSI TRIGGER: RSI(14) was BELOW its own {CFG['rsi_avg_period']}-period
     moving average yesterday — confirms it's approaching FROM
     BELOW — AND today it's either very near that average (within
     near_tolerance points) or has already crossed above it.
  2) AVERAGE RSI STRONG: RSI's own moving average is currently
     above rsi_avg_min_level ({CFG['rsi_avg_min_level']:.0f}).
  3) STRUCTURE: price above SMA50 AND SMA150, AND SMA50 above
     SMA150.
  ...
  📋 SCORE (0-100)
  RSI-gap tightness (0-30) + RSI-avg strength above 50 (0-25) +
  distance price sits above SMA50 (0-15) + freshness (0-15) + base
  points for clearing every gate (15)

  💡 BEST SETUPS
  Score > 70               tight RSI gap, strong avg, fresh, well
                            above SMA50
  RSI_Gap near 0               right at the cross, neither too early
                                nor already extended
  RSI_Avg well above 50           genuinely healthy momentum backdrop
  Days_Since_Signal = 0-3            freshest signal

  ⚙️  TUNE IF 0 RESULTS
  near_tolerance                2.0 → 4.0   (allow a wider RSI gap)
  rsi_avg_min_level            50.0 → 45.0
